...

{
    ...
    //caculate the number need to be rotated in bits 
    numBits = (index+countBits(*c))%8;

    // perform circular rotation of the char
    // odd 1s in bits rotate to right
    // even 1s in bits rotate to left
    if(countBits(*c)%2){
...

{
    ...
    *c = *c^key;
    ...
    // caculate the number of bits need to be rotated
    numBits = (index+countBits(*c))%8;
    
    
    // perform circular rotation of the char
    // the rules opposite with what happens in encryptChar
    if(countBits(*c)%2){
        rotateLeft(c,numBits);
    }else{
        rotateRight(c,numBits);
