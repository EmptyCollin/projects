    ...
    if shop.clicked_item is None:
        ...
        for item in shop.shop_data:
            ...
            if (mX > shop.shop_data[item]["location"][0] and mX < shop.shop_data[item]["location"][0] + shop.ui_data["item_size"]) and (mY > shop.shop_data[item]["location"][1] and mY < shop.shop_data[item]["location"][1] + shop.ui_data["item_size"]):
                ...
                if clicked and shop.shop_data[item]["available"]:
                    shop.clicked_item = item
    if not clicked:
        returnItem = shop.clicked_item
        shop.clicked_item = None
        if returnItem is not None:
            return {"Type": returnItem, "Location": [pygame.mouse.get_pos()[0] - shop.ui_data["item_size"] // 2, pygame.mouse.get_pos()[1] - shop.ui_data["item_size"] // 2], "Cost": shop.shop_data[returnItem]["cost"], "Radius Sprite": shop.ui_data["radius_sprite"] }

def render_shop(shop, screen, settings, current_currency,current_kill):
    ...
    # Handle Player Current kill
    current_kill_text = settings.font.render("Killed Aliens: " + str(current_kill), True, (254, 207, 0))
    screen.blit(current_kill_text, (shop.location[0] + 5, 725))
